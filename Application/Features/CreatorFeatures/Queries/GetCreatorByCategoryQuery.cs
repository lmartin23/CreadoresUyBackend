...
{
    public class GetCreatorByCategoryQuery : IRequest<Response<List<CreadorSearchDto>>>
    {
        public string SearchCategory {  get; set; }
        public int SizePage { get; set; }
        public int Page { get; set; }
        public class GetCreatorByCategoryQueryHandler : IRequestHandler<GetCreatorByCategoryQuery, Response<List<CreadorSearchDto>>>
        {
            private readonly ICreadoresUyDbContext _context;
            private readonly IMapper _mapper;

            public GetCreatorByCategoryQueryHandler(ICreadoresUyDbContext context, IMapper mapper)
            {
                _context = context;
                _mapper = mapper;
            }
            public async Task<Response<List<CreadorSearchDto>>> Handle(GetCreatorByCategoryQuery query, CancellationToken cancellationToken)
            {

                var creatorList = await _context.Creators.Where(c => (c.Category1.Contains(query.SearchCategory) || c.Category2.Contains(query.SearchCategory)) && c.Deleted==false).Include(c =>  c.Plans).ThenInclude(c=>c.UserPlans).Skip(query.Page * query.SizePage).Take(query.SizePage).ToListAsync();
